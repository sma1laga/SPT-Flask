...
# Frequency
W_MIN, W_MAX, W_N = -10*np.pi, 10*np.pi, 4001
...
def Y_freq(w, w_g, w_a):
    ...
    return Xa_freq(w, w_g, w_a) * ((2 * np.pi) / w_a) * lpf
...
def reconstruct_y(t, T, w_g, x_samples, t_samples, partials=False):
    ...
    if partials:
        ...
        lo = max(0, idx_center-3)
        hi = min(Ns, idx_center+4)
...
def _render_cached(
    ...
    with plt.rc_context(RC_PARAMS):
        fig, (axT, axF) = plt.subplots(1, 2, figsize=(8.3, 3), layout="constrained")

        # Time domain
        axT.set_title("Time domain")
        axT.set_xlabel(r"$t$")
        axT.set_ylabel(r"$x(t),x_\mathrm{a}(t),y(t)$")
        axT.set_xlim(T_MIN, T_MAX)
        axT.set_ylim(-0.7, 2.7)
        axT.grid(True)

        if show_xt:
            axT.plot(t_grid, xt, color="black", lw=2, label=r"$x(t)$")
        if show_xa:
            if ts.size:
                axT.vlines(ts, 0, xa_vals, colors="tab:blue")
                axT.plot(ts, xa_vals, "^", color="tab:blue", label=r"$x_\mathrm{a}(t)$")
        if show_yt:
            axT.plot(t_grid, yt, color="tab:red", lw=2, ls="--", label=r"$y(t)$")
        if show_partials and parts:
            n_parts = len(parts)
            for i, p in enumerate(parts):
                # alpha fades out for parts away from center
                axT.plot(t_grid, p, color="tab:green", lw=1.2, ls="--", alpha=(n_parts//2+1 - np.abs(i-n_parts//2)) / (n_parts//2+1))
        ...
        axF.set_ylabel(r"$X(\mathrm{j}\omega),X_\mathrm{a}(\mathrm{j}\omega),Y(\mathrm{j}\omega)$")
        axF.set_xlim(W_MIN, W_MAX)
        axF.set_ylim(-0.2, 4.2)
        # pi ticks
        k0 = int(np.ceil(W_MIN/np.pi))
        k1 = int(np.floor(W_MAX/np.pi))
        ...
        if show_X:
            axF.plot(w_grid, Xw, color="black", lw=2, label=r"$X(\mathrm{j}\omega)$", solid_joinstyle="bevel")
        if show_Xa:
            alpha = 0.6 if show_X else 1
            axF.plot(w_grid, Xaw, color="tab:blue", alpha=alpha, label=r"$X_\mathrm{a}(\mathrm{j}\omega)$", solid_joinstyle="bevel")
        if show_Y:
            axF.plot(w_grid, Yw, color="tab:red", lw=2, ls="--", label=r"$Y(\mathrm{j}\omega)$", solid_joinstyle="bevel")

"""Delay Estimation (frequency domain) demo"""
...
demos_delay_estimation_freq_bp = Blueprint(
    "demos_delay_estimation_freq", __name__, template_folder="../../templates"
)
...
def _compute_cpsd(x: np.ndarray, y: np.ndarray):
    ...
    magnitude_db = 10 * np.log10(np.abs(pxy) + np.finfo(float).eps)
    phase = np.angle(pxy) / np.pi
    return freqs.astype(np.float32), magnitude_db.astype(np.float32), phase.astype(np.float32)
...
def _prepare_signals(signal_kind: str, snr_db: float, delay_ms: float):
    ...
    freqs, cpsd_mag_db, cpsd_phase = _compute_cpsd(sliced2, sliced1)
...
@demos_delay_estimation_freq_bp.route("/", methods=["GET"])
def page():
    defaults = {
        "snr": DEFAULT_SNR,
    ...
    return render_template("demos/delay_estimation_frequency.html", defaults=defaults)


@demos_delay_estimation_freq_bp.route("/compute", methods=["POST"])
def compute():
    ...
    return jsonify(
        {
            ...
            "cpsd_mag_db": data["cpsd_mag_db"].tolist(),
            "cpsd_phase": data["cpsd_phase"].tolist(),
            "stats": {
